...
NOT a pytest suite, despite what the old filename and the ``test_``
prefixes implied. These are interactive probes that bind UDP 8765, so
pytest collected them on every ``make test``: they failed with
``OSError: [Errno 98] Address already in use`` whenever the controller was
running, and blocked on a socket read when it was not. Renamed to
``udp_probe.py`` with ``probe_`` prefixes so test discovery leaves them
alone.

Run it by hand:  python udp_probe.py [listen|send|bidirectional]
"""
...
def probe_receive_commands(port: int = 8765, timeout: float = 10.0) -> None:
    """
...
def probe_send_dummy_command(host: str = "127.0.0.1", port: int = 8765) -> None:
    """
...
def probe_bidirectional(port: int = 8765) -> None:
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if mode == "listen":
            probe_receive_commands()
        elif mode == "send":
            probe_send_dummy_command()
        elif mode == "bidirectional":
            probe_bidirectional()
        else:
            print(f"Unknown mode: {mode}")
            print("Usage: python udp_probe.py [listen|send|bidirectional]")
    else:
        print("Modes:")
        ...
        print()
        probe_receive_commands(timeout=30.0)
